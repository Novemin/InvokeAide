# Google Tasks / Calendar API 実機検証 — 受け入れ基準 v0.1
...
**公式ドキュメント予測**: `due` フィールドは日付情報のみ、 時刻部分は **破棄** される
...
**公式ドキュメント予測**: Task resource に `deadline` フィールドは **存在しない**
...
**公式ドキュメント予測**: Event resource に `taskSeries` フィールドは **存在しない**(反復系は `recurrence` / `recurringEventId` が標準)
...
「公式ドキュメントが言う事実」 と「実機の挙動」 が **一致しない** 状態。 例:
...
4. **対策案を提示** — 仕様変更時: 「ハイブリッド (ID 連携) 方式への切替検討」 / バージョン差時: 「googleapis ライブラリのバージョン固定検討」 / 公式記述更新時: 「公式 issue tracker への報告検討」
...
- v0.1(本書): 起草、 Sさん 検証実行前
- v0.2 候補: Sさん 検証実行 + Tさん 結果文書化後、 食い違いに応じて受け入れ基準を更新
- v0.3 候補: B2 実装着手後、 §5 仕様§1.3 依存項目を本実装テスト基準に格上げ
